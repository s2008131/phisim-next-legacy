...
class Stat {
	...
	getData(isAuto, speed = '') {
		...
		const scoreDeltaRaw = Math.abs(scoreBest - this.scoreStr).toString().padStart(7, '0');
		...
		if (isAuto) return Object.assign(pbj, { newBestColor: '#fff', newBestStr: 'AUTOPLAY', scoreDelta: '' });
...
class Renderer {
	constructor(stage) {
		...
		this.isFull = false;
		console.log('Hello, Phigros Simulator [Next]!');
		//qwq
		this.speed = 1;
		// this.config = {};
		this.chart = {};
