...
self._i = ['Phi\x67ros 模拟器', ['0x1D'], 1611795955, 1682347041];
...
async function checkSupport() {
	...
	if (!await loadLib('ogg格式兼容', '../static/lib/oggmented-bundle.js', () => !oggCompatible && isUndefined('oggmented'))) return -4;
...
const stage = {
	...
	resize(ratio) {
		...
		const stageWidth = app.stage.offsetWidth;
		const stageHeight = stageWidth / this.aspectRatio;
		if (app.isFull) app.stage.style.cssText = ';position:fixed;top:0;left:0;bottom:0;right:0;width:100%;';
		else app.stage.style.cssText = `;height:${stageHeight.toFixed()}px;width:90%`;
	}
};
window.onload = function() {stage.resize(1.4316546762589928)}; //qwq
...
await Promise.all(Object.entries({
	JudgeLine: '../static/src/JudgeLine.png',
	ProgressBar: '../static/src/ProgressBar.png',
	SongsNameBar: '../static/src/SongsNameBar.png',
	Pause: '../static/src/PauseNew.png',
	HitFXRaw: '../static/src/clickRaw.png',
	Tap: '../static/src/Tap.png',
	Tap2: '../static/src/Tap2.png',
	TapHL: '../static/src/TapHL.png',
	Drag: '../static/src/Drag.png',
	DragHL: '../static/src/DragHL.png',
	HoldHead: '../static/src/HoldHead.png',
	HoldHeadHL: '../static/src/HoldHeadHL.png',
	Hold: '../static/src/Hold.png',
	HoldHL: '../static/src/HoldHL.png',
	HoldEnd: '../static/src/HoldEnd.png',
	Flick: '../static/src/Flick.png',
	FlickHL: '../static/src/FlickHL.png',
	LevelOver1: '../static/src/LevelOver1.png',
	LevelOver3: '../static/src/LevelOver3.png',
	LevelOver4: '../static/src/LevelOver4.png',
	LevelOver5: '../static/src/LevelOver5.png',
	LevelOver0_v2: '../static/src/LevelOver0.ogg',
	LevelOver1_v2: '../static/src/LevelOver1.ogg',
	LevelOver2_v2: '../static/src/LevelOver2.ogg',
	LevelOver3_v2: '../static/src/LevelOver3.ogg',
	Rank: '../static/src/Rank.png',
	NoImage: '../static/src/0.png',
	mute: '../static/src/mute.ogg',
	HitSong0: '../static/src/HitSong0.ogg',
	HitSong1: '../static/src/HitSong1.ogg',
	HitSong2: '../static/src/HitSong2.ogg',
	FCV: '../static/src/v2/FCV.png'
...
function mainLoop() {
	...
	if (!window.spec.isPhiTogetherApp) {
		watermarkYPos = canvas.height - lineScale * 0.2;
	} else {
		watermarkYPos = canvas.height - lineScale * 0.5;
	}
	ctx.fillText(`Phigros Simulator [Next] Public - ${_i[1]} by lchzh3473 / kev1nweng / scar922${tmps.wmisAutoStr}`, canvas.width / 2 - lineScale * 0, watermarkYPos);
}
...
function loopNoCanvas() {
	...
	// tmps.watermark = app.playMode === 1 ? `Simulated Gameplay` : '';
	tmps.watermark = app.playMode === 1 ? '' : '';
	tmps.wmisAutoStr = app.playMode === 1 ? "- AUTO MODE" : '';
}
...
function loopCanvas() { //尽量不要在这里出现app
	...
	ctxos.fillRect(0, 0, canvasos.width, canvasos.height);
	// if (qwq[4]) ctxos.filter = `hue-rotate(${stat.combo*360/7}deg)`;
	ctxos.font = `${lineScale * 1.5}px Saira, SyHybrid, Noto Sans SC`;
	let textWidth = ctxos.measureText(tmps.watermark).width;
	if (textWidth > (canvasos.width / 1.5)) {
		ctxos.font = `${lineScale * 1.5 * ((canvasos.width / 1.5) / textWidth)}px Saira, SyHybrid, Noto Sans SC`;
	} else {
		ctxos.font = `${lineScale * 1.5}px Saira, SyHybrid, Noto Sans SC`;
	}
	ctxos.fillStyle = '#FFF';
	if ( qwqIn.second > 3 ) {
		ctxos.globalAlpha = 0.025;
	} else {
		ctxos.globalAlpha = 0;
	}
	ctxos.textAlign = 'center';
	ctxos.fillText((tmps.watermark), canvasos.width / 2, canvasos.height / 2 + lineScale * 0.75);
...
function qwqdraw3(statData) {
	...
	ctxos.globalAlpha = 1;
	if ($id('imageBlur').checked) ctxos.drawImage(app.bgImageBlur, ...adjustSize(app.bgImageBlur, canvasos, 1));
...
status.reg('showIntro', $id('showIntro'));
status.reg('showFPS', $id('showFPS'));
...
const updateLevelText = type => {
	const table = { SP: [0, 0], EZ: [1, 7], HD: [3, 12], IN: [6, 15], AT: [13, 17] };
...
const autoIndicatorElem = $id('autoIndicator');
function toggleAutoIndicator() {
	if ($id("autoplay").checked) {
		autoIndicatorElem.innerHTML = "ON";
		autoIndicatorElem.style = "color: forestgreen;";
	} else {
		autoIndicatorElem.innerHTML = "OFF";
		autoIndicatorElem.style = "color: red;";
	}
}
toggleAutoIndicator()
